...
class SAC(OffPolicyAlgorithm):
    ...
    :param use_amp: Whether or not to use automatic mixed precision (AMP) for the training (fp16). Only useful for certain GPUs (e.g. RTX series)
    """
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            if ent_coef_loss is not None and self.ent_coef_optimizer is not None:
                ...
                self.grad_scaler.scale(ent_coef_loss).backward()
                self.grad_scaler.step(self.ent_coef_optimizer)
            ...
            # Check if scale is below min scale value
            # Based on solution from https://github.com/pytorch/pytorch/issues/40497
            if self.grad_scaler._scale < self.min_scale_value:
                self.grad_scaler._scale = torch.tensor(self.min_scale_value).to(self.grad_scaler._scale)
